...
function newName(num){
	...
	for(var i=0;i<num;i++){
		a=i%10;
		b=i%12;
		names[i]=xing[i]+tiangan[a]+dizhi[b]
	}
	names=names.join(',');
	return names;
}
function newScore(num,full){
	var scores=[];
	for(var i=0;i<num;i++){
		scores[i]=Math.floor(Math.random()*full);
	}
	scores=scores.join(',');
	return scores;
}